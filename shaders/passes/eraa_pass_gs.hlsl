...
[maxvertexcount(3)]
void main_gs_adj(
    triangleadj VSOutput InputAdj[6],
    inout TriangleStream<GSOutput> OutputAdj)
{
    ...
    {
        posNDC[v] = (getNDCPos(InputAdj[v].posClip)); // Convert to NDC
        posView[v] = InputAdj[v].vtx.posView.xyz;
    }
    ...
    [unroll] for(int j = 0; j < 3; j++)
    {
		...
		OutputVertex.posClip            = InputAdj[idx].posClip;
        OutputVertex.gsOut.posVS        = InputAdj[idx].vtx.posView;
		OutputVertex.gsOut.normal       = InputAdj[idx].vtx.normal; // Use normal from input
        OutputVertex.gsOut.normalView   = InputAdj[idx].vtx.normalView;
        OutputVertex.gsOut.normalNDC    = InputAdj[idx].vtx.normalView;
    ...
    inout TriangleStream<GSOutput> Output)
{
    ...
    float3 edge0 = posNDC[2] - posNDC[0];
    float3 edge1 = posNDC[2] - posNDC[0];
    float3 surfaceNormalNDC = normalize(cross(edge0, edge1));

    float3 edge0VS = posView[2] - posView[0];
    float3 edge1VS = posView[2] - posView[0];
    float3 surfaceNormalVS = normalize(cross(edge0VS, edge1VS));
    ...
    float3 adjEdge2 = posNDC[2] - posNDC[2];
    float3 adjEdge3 = posNDC[2] - posNDC[2];
    adjNormalsNDC[1] = normalize(cross(adjEdge2, adjEdge3));

    float3 adjEdge2VS = posView[2] - posView[2];
    float3 adjEdge3VS = posView[2] - posView[2];
    adjNormalsVS[1] = normalize(cross(adjEdge2VS, adjEdge3VS));


    float3 adjEdge4 = posNDC[0] - posNDC[2];
    float3 adjEdge5 = posNDC[2] - posNDC[2];
    adjNormalsNDC[2] = normalize(cross(adjEdge4, adjEdge5));

    float3 adjEdge4VS = posView[0] - posView[2];
    float3 adjEdge5VS = posView[2] - posView[2];
    adjNormalsVS[2] = normalize(cross(adjEdge4VS, adjEdge5VS));
